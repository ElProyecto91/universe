...
export default function Compatibilidad() {
  ...
  const calcular = async () => {
    ...
    // Cacheable por par de fechas ordenado (independiente del nombre)
    ...
    try {
      ...
      if (cached?.respuesta) {
        // Personalizar con los nombres reales
        const textoPersonalizado = cached.respuesta.replace(/Persona 1/g, nombre1).replace(/Persona 2/g, nombre2)
        setInterpretacion(textoPersonalizado)
        setFromCache(true)
        setCargando(false)
        return
      }
    } catch (err) { console.warn('[Compatibilidad] Error caché:', err) }

    const result = await llamarGemini({
      herramienta: 'compatibilidad',
      prompt: `Experta en numerología y compatibilidad entre personas.
...
Desafío: ${res.conexion.desafio} · Potencial: ${res.conexion.potencial}

3-4 párrafos: energía individual de cada número, aportación de cada persona, significado del número de conexión, consejo práctico. Equilibrado — explora potencial y desafíos sin afirmar que funciona o no.`,
      userId, usarLite: false, cacheable: false, maxTokens: 450,
    })

    if (!result.error && result.texto) {
      setInterpretacion(result.texto)
      supabase.from('ai_cache').insert({ cache_key: cacheKey, herramienta: 'compatibilidad', prompt_hash: cacheKey, respuesta: result.texto, tokens_used: result.tokensUsados, expires_at: null }).then(() => {})
    } else setInterpretacion('Los números guardan silencio. Inténtalo de nuevo.')
    setCargando(false)
  }
    ...
    <div className="min-h-screen text-white flex flex-col relative" style={bgStyle}>
      ...
      <div className="relative z-10 w-full max-w-sm mx-auto flex flex-col px-6 py-10 gap-6">
        <div className="flex items-center">
          <button onClick={() => { if (fase === 'resultado') setFase('datos'); else window.location.href = '/tradiciones' }} className="text-purple-300 text-sm">← Volver</button>
        ...
        {fase === 'datos' && (
          <div className="flex flex-col gap-5">
            ...
            <button onClick={calcular} disabled={!nombre1 || !fecha1 || !nombre2 || !fecha2} className="w-full bg-gradient-to-r from-purple-600 to-pink-600 text-white font-semibold py-4 rounded-full hover:opacity-90 transition disabled:opacity-40">Explorar compatibilidad</button>
          </div>
        )}
          ...
          <div className="flex flex-col gap-5">
            <div className="bg-white/5 border border-white/10 rounded-3xl p-6 backdrop-blur">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="text-center px-4"><p className="text-white/30 text-2xl">+</p></div>
                <div className="text-center flex-1">
            ...
            <div className="bg-white/5 border border-white/10 rounded-3xl p-6 backdrop-blur">
              <div className="flex items-center justify-between mb-3">
                <p className="text-purple-300 text-xs tracking-widest uppercase">Lectura profunda</p>
            ...
            {!cargando && interpretacion && <Compartir titulo={`Compatibilidad: ${resultado.persona1.nombre} y ${resultado.persona2.nombre}`} texto={`Número de Conexión ${resultado.numeroConexion}: ${resultado.conexion.titulo}\n\n${interpretacion}`} hashtags={['Compatibilidad', 'Universe', 'Numerologia', 'Amor']} />}
            <div className="flex flex-col gap-3">
              ...
              <button onClick={() => { setFase('datos'); setInterpretacion(''); setResultado(null); setFromCache(false) }} className="w-full text-purple-300/60 text-sm py-2">Nueva compatibilidad</button>
            </div>
          </div>
        )}
      </div>
