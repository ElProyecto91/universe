...
import { calcularZiWei } from '../lib/motores/ziwei'
...
export default function ZiWei() {
  ...
  const cacheKey = `zi-wei-${fechaNacimiento}-${añoActual}`
  const userId = null
  ...
  const generarLectura = async () => {
    ...
    } catch (err) { console.warn('[ZiWei] Error caché:', err) }

    const result = await llamarGemini({
      herramienta: 'zi-wei',
      prompt: `Experto en Zi Wei Dou Shu. Nombre: ${nombre}, Signo: ${signo}, Nacimiento: ${fechaNacimiento}. Genera una lectura profunda de 3-4 párrafos. Reflexivo, simbólico. Sin predicciones absolutas. Máximo 400 palabras.`,
    ...
    if (!result.error && result.texto) {
      ...
      setFromCache(false)
      supabase.from('ai_cache').insert({ cache_key: cacheKey, herramienta: 'zi-wei', prompt_hash: cacheKey, respuesta: result.texto, tokens_used: result.tokensUsados, expires_at: null }).then(() => {})
    } else {
      ...
      <div className="relative z-10 w-full max-w-sm mx-auto flex flex-col px-6 py-10 gap-6">
        <div className="flex items-center">
          ...
          <div className="flex-1 text-center">
            ...
            <p className="text-purple-300 text-xs">Astrología china imperial</p>
          </div>
        </div>
        <div className="bg-white/5 border border-white/10 rounded-3xl p-6 backdrop-blur text-center">
          <p className="text-purple-300 text-xs tracking-widest uppercase mb-2">Zi Wei Dou Shu</p>
        ...
        {!cargando && interpretacion && <Compartir titulo="Zi Wei Dou Shu" texto={interpretacion} hashtags={['Universe', 'ZiWei']} />}
