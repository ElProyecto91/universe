...
export default function ColorOracle() {
  ...
  const { registrarApertura, registrarLectura, registrarValoracion } = useAnalytics('color-oracle', esPremium)
  ...
  const generarLectura = async () => {
    ...
    } catch (err) { console.warn('[ColorOracle]', err) }
    ...
    if (!result.error && result.texto) {
      ...
      supabase.from('horoscopo_cache').insert({
        signo: signo.toLowerCase(), fecha: fechaHoy, tipo: 'color-oracle',
    ...
    <div className="min-h-screen text-white flex flex-col relative" style={bgStyle}>
      ...
      <div className="relative z-10 w-full max-w-sm mx-auto flex flex-col px-6 py-10 gap-6">
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-5 backdrop-blur text-center">
          <p className="text-purple-300 text-xs tracking-widest uppercase mb-1">Color Oracle</p>
