...
export default function ChakraOracle() {
  ...
  const { registrarApertura, registrarLectura, registrarPaywall, registrarValoracion } = useAnalytics('chakra-oracle', esPremium)
  ...
  if (!cargandoPlan && !esPremium) {
    registrarPaywall()
    return <Paywall motivo="herramienta" herramienta="Chakra Oracle" />
  }
  ...
  const consultar = async () => {
    ...
    const result = await llamarGemini({
      ...
      prompt: `Experto en chakras (tradición hindú adoptada en espiritualidad occidental). Nombre: ${nombre}. Situación: "${pregunta}". 3 párrafos: chakra resonante, señales, práctica.`,
