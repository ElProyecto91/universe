...
import { calcularPlantOracle } from '../lib/motores/plantoracle'
...
export default function PlantOracle() {
  ...
  const cacheKey = `plant-oracle-${fechaNacimiento}-${añoActual}`
  const userId = null
  ...
  const generarLectura = async () => {
    ...
    } catch (err) { console.warn('[PlantOracle] Error caché:', err) }

    const result = await llamarGemini({
      herramienta: 'plant-oracle',
      prompt: `Experto en Oracle de Plantas. Nombre: ${nombre}, Signo: ${signo}, Nacimiento: ${fechaNacimiento}. Genera una lectura profunda de 3-4 párrafos. Reflexivo, simbólico. Sin predicciones absolutas. Máximo 350 palabras.`,
    ...
    if (!result.error && result.texto) {
      ...
      setFromCache(false)
      supabase.from('ai_cache').insert({ cache_key: cacheKey, herramienta: 'plant-oracle', prompt_hash: cacheKey, respuesta: result.texto, tokens_used: result.tokensUsados, expires_at: null }).then(() => {})
    } else {
    ...
    <div className="min-h-screen text-white flex flex-col relative" style={bgStyle}>
      ...
      <div className="relative z-10 w-full max-w-sm mx-auto flex flex-col px-6 py-10 gap-6">
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-6 backdrop-blur text-center">
          <p className="text-purple-300 text-xs tracking-widest uppercase mb-2">Oracle de Plantas</p>
        ...
        {!cargando && interpretacion && <Compartir titulo="Oracle de Plantas" texto={interpretacion} hashtags={['Universe', 'PlantOracle']} />}
