...
const HERRAMIENTA = 'vision-board'
...
export default function VisionBoard() {
  ...
  const consultar = async () => {
    ...
    try {
      const prompt = [
        'Eres una guía de manifestación y visualización creativa.',
        'Responde SOLO con texto en español, en prosa continua. Sin asteriscos, sin guiones, sin numeración, sin markdown.',
        '',
        `El usuario se llama ${nombre}, signo ${signo}. Su intención o deseo es: "${pregunta}".`,
        '',
        'Escribe una guía de visualización de 3 párrafos.',
        'Párrafo 1: describe cómo visualizar esta intención ya cumplida, con todos los detalles sensoriales.',
        'Párrafo 2: describe 3 acciones concretas y sencillas que puede hacer hoy para anclar esta manifestación.',
        'Párrafo 3: un mensaje de cierre que refuerce la creencia en esta visión.',
        '',
        'Tono inspirador y directo. Separa párrafos con línea en blanco. Termina en punto.',
      ].join('\n')
      ...
      if (!result.error && result.texto) {
        ...
        if (!lecturaGuardadaRef.current) {
          ...
          await guardarLectura({ herramienta: HERRAMIENTA, titulo: `Vision Board · ${fechaHoy}`, contenido: `Intención: "${pregunta}"\n\n${result.texto}`, metadatos: { pregunta, fecha: fechaHoy, nombre, signo } })
        }
      } else { setErrorMsg(result.error || 'El universo guarda silencio. Inténtalo de nuevo.') }
    } catch (err) { console.error('[VisionBoard]', err); setErrorMsg('Error inesperado.') }
      ...
      <div className="flex flex-col gap-6">
        <div className="flex items-center">
          ...
          <div className="flex-1 text-center">
            ...
            <p className="text-purple-300 text-xs">Tu tablero de manifestación</p>
          </div>
        ...
        {fase === 'preguntar' && (
          <div className="flex flex-col gap-4">
            <div className="bg-[#0d0015] border border-white/15 rounded-3xl p-6">
              ...
              <textarea value={pregunta} onChange={e => setPregunta(e.target.value)} placeholder="¿Qué quieres manifestar?" rows={4} className="w-full bg-transparent text-white text-sm resize-none outline-none placeholder-white/30" />
            </div>
            <DisclaimerIA compact />
            <button onClick={consultar} disabled={!pregunta.trim()} className="w-full bg-gradient-to-r from-purple-600 to-pink-600 text-white font-semibold py-4 rounded-full hover:opacity-90 transition disabled:opacity-40">Visualizar</button>
          </div>
        )}
        {fase === 'resultado' && (
          <div className="flex flex-col gap-4">
            ...
            <div className="bg-[#0d0015] border border-white/15 rounded-3xl p-6">
              <p className="text-purple-400 text-xs tracking-widest uppercase mb-4">Tu visión</p>
              {cargando ? (
            ...
            {!cargando && interpretacion && (
              <>
                ...
                <Compartir titulo="Vision Board" texto={interpretacion} hashtags={['Universe', 'VisionBoard']} />
                ...
                <button onClick={() => { setFase('preguntar'); setInterpretacion(''); setErrorMsg(''); lecturaGuardadaRef.current = false }} className="w-full text-purple-300/60 text-sm py-2">Nueva visión</button>
              </>
            )}
          </div>
        )}
