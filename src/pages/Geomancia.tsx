...
export default function Geomancia() {
  ...
  const consultar = async () => {
    ...
    try {
      const base = `Eres un experto en geomancia árabe (ilm al-raml). El usuario se llama ${nombre}, signo ${signo}. Su pregunta: "${pregunta}". Escribe en español, en prosa, sin listas ni asteriscos. Exactamente 3 frases seguidas. Sin saludar ni usar el nombre al inicio.`

      const r1 = await llamarGemini({ herramienta: HERRAMIENTA, prompt: `${base} Escribe un párrafo sobre la situación actual según la geomancia: qué energías están presentes y qué refleja el momento.`, userId: userPlan.userId, usarLite: true, cacheable: false, maxTokens: 250 })
      if (r1.error) { setErrorMsg('El universo guarda silencio. Inténtalo de nuevo.'); return }

      await new Promise(r => setTimeout(r, 500))
      const r2 = await llamarGemini({ herramienta: HERRAMIENTA, prompt: `${base} Ya escribiste: "${r1.texto.trim()}". Continúa con un párrafo sobre cómo se desarrollará esta situación y qué fuerzas influyen en el resultado.`, userId: userPlan.userId, usarLite: true, cacheable: false, maxTokens: 250 })
      if (r2.error) { setErrorMsg('El universo guarda silencio. Inténtalo de nuevo.'); return }

      await new Promise(r => setTimeout(r, 500))
      const r3 = await llamarGemini({ herramienta: HERRAMIENTA, prompt: `${base} Ya escribiste: "${r1.texto.trim()} ${r2.texto.trim()}". Cierra con un párrafo de consejo práctico y una pregunta reflexiva de cierre.`, userId: userPlan.userId, usarLite: true, cacheable: false, maxTokens: 250 })
      ...
      if (!lecturaGuardadaRef.current) {
        ...
        await guardarLectura({ herramienta: HERRAMIENTA, titulo: `Geomancia · ${fechaHoy}`, contenido: `Consulta: "${pregunta}"\n\n${texto}`, metadatos: { pregunta, fecha: fechaHoy, nombre, signo } })
      }
    } catch (err) { console.error('[Geomancia]', err); setErrorMsg('Error inesperado.') }
      ...
      <div className="flex flex-col gap-6">
        <div className="flex items-center">
          ...
          <div className="flex-1 text-center">
            ...
            <p className="text-purple-300 text-xs">Tradición árabe-medieval</p>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            {!cargando && interpretacion && (
              <>
                ...
                <Compartir titulo="Geomancia · علم الرمل" texto={interpretacion} hashtags={['Universe', 'Geomancia']} />
