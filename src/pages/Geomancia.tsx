...
export default function Geomancia() {
  ...
  const { registrarApertura, registrarLectura, registrarPaywall, registrarValoracion } = useAnalytics('geomancia', esPremium)
  ...
  if (!cargandoPlan && !esPremium) {
    registrarPaywall()
    return <Paywall motivo="herramienta" herramienta="Geomancia · علم الرمل" />
  }
  ...
  const consultar = async () => {
    ...
    const result = await llamarGemini({
      ...
      prompt: `Experto en geomancia (ilm al-raml). Nombre: ${nombre}. Pregunta: "${pregunta}". 3 párrafos: situación actual, desarrollo, consejo.`,
