...
export default function Palmisteria() {
  ...
  const consultar = async () => {
    ...
    try {
      const base = `Eres un experto en palmistería. El usuario se llama ${nombre}, signo ${signo}, nacido el ${fechaNacimiento}. Área de vida consultada: "${pregunta}". Responde SOLO con texto en español, en prosa continua. Sin asteriscos, sin guiones, sin markdown. Exactamente 3 frases terminadas en punto.`

      const r1 = await llamarGemini({ herramienta: HERRAMIENTA, prompt: `${base} Describe qué revela la línea principal relacionada con esta área de vida y qué energía dominante muestra.`, userId: userPlan.userId, usarLite: true, cacheable: false, maxTokens: 250 })
      if (r1.error) { setErrorMsg('El universo guarda silencio. Inténtalo de nuevo.'); return }

      await new Promise(r => setTimeout(r, 500))
      const r2 = await llamarGemini({ herramienta: HERRAMIENTA, prompt: `${base} Continuando desde: "${r1.texto.trim()}" — describe qué matices o líneas secundarias complementan esta lectura y qué revelan sobre los talentos o desafíos de ${nombre}.`, userId: userPlan.userId, usarLite: true, cacheable: false, maxTokens: 250 })
      if (r2.error) { setErrorMsg('El universo guarda silencio. Inténtalo de nuevo.'); return }

      await new Promise(r => setTimeout(r, 500))
      const r3 = await llamarGemini({ herramienta: HERRAMIENTA, prompt: `${base} Continuando desde: "${r1.texto.trim()} ${r2.texto.trim()}" — da un consejo práctico basado en esta lectura y una pregunta reflexiva de cierre para ${nombre}.`, userId: userPlan.userId, usarLite: true, cacheable: false, maxTokens: 250 })
      ...
      if (!lecturaGuardadaRef.current) {
        ...
        await guardarLectura({ herramienta: HERRAMIENTA, titulo: `Palmistería · ${fechaHoy}`, contenido: `Consulta: "${pregunta}"\n\n${texto}`, metadatos: { pregunta, fecha: fechaHoy, nombre, signo } })
      }
    } catch (err) { console.error('[Palmisteria]', err); setErrorMsg('Error inesperado.') }
        ...
        {fase === 'preguntar' && (
          <div className="flex flex-col gap-4">
            <div className="bg-[#0d0015] border border-white/15 rounded-3xl p-6">
              ...
              <textarea value={pregunta} onChange={e => setPregunta(e.target.value)} placeholder="¿Qué área de tu vida quieres explorar?" rows={4} className="w-full bg-transparent text-white text-sm resize-none outline-none placeholder-white/30" />
            </div>
