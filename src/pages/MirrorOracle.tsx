...
export default function MirrorOracle() {
  ...
  const { registrarApertura, registrarLectura, registrarPaywall, registrarValoracion } = useAnalytics('mirror-oracle', esPremium)
  ...
  if (!cargandoPlan && !esPremium) {
    registrarPaywall()
    return <Paywall motivo="herramienta" herramienta="Mirror Oracle" />
  }
  ...
  const consultar = async () => {
    ...
    const result = await llamarGemini({
      ...
      prompt: `Eres un guía de reflexión profunda. Nombre: ${nombre}, Signo: ${signo}. Situación: "${pregunta}". 3 párrafos reflexivos. Sin predicciones absolutas.`,
