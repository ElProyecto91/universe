...
const HERRAMIENTA = 'omens-oracle'
...
export default function OmensOracle() {
  ...
  const consultar = async () => {
    ...
    try {
      const prompt = [
        'Eres un experto en sincronicidades, presagios y simbolismo en distintas tradiciones culturales.',
        'Escribe en español, en prosa, como una carta continua. Sin listas, sin asteriscos, sin markdown.',
        'No empieces con el nombre del usuario ni con saludos.',
        '',
        `El usuario se llama ${nombre}, signo ${signo}. Ha observado esta señal o presagio: "${pregunta}".`,
        '',
        'Escribe 2 párrafos que fluyan naturalmente.',
        'Párrafo 1: qué simboliza esta señal en distintas tradiciones culturales y espirituales, y qué mensaje trae.',
        'Párrafo 2: cómo conecta con el momento actual de esta persona y qué invita a explorar o hacer.',
        '',
      ...
      if (!result.error && result.texto) {
        ...
        if (!lecturaGuardadaRef.current) {
          ...
          await guardarLectura({ herramienta: HERRAMIENTA, titulo: `Oracle de Presagios · ${fechaHoy}`, contenido: `Señal: "${pregunta}"\n\n${result.texto}`, metadatos: { pregunta, fecha: fechaHoy, nombre, signo } })
        }
      } else { setErrorMsg(result.error || 'El universo guarda silencio. Inténtalo de nuevo.') }
    } catch (err) { console.error('[OmensOracle]', err); setErrorMsg('Error inesperado.') }
        ...
        {fase === 'preguntar' && (
          <div className="flex flex-col gap-4">
            <div className="bg-[#0d0015] border border-white/15 rounded-3xl p-6">
              <p className="text-purple-400 text-xs tracking-widest uppercase mb-3">Tu señal o presagio</p>
              <textarea value={pregunta} onChange={e => setPregunta(e.target.value)} placeholder="Describe la señal que observaste..." rows={4} className="w-full bg-transparent text-white text-sm resize-none outline-none placeholder-white/30" />
            </div>
            <DisclaimerIA compact />
            <button onClick={consultar} disabled={!pregunta.trim()} className="w-full bg-gradient-to-r from-purple-600 to-pink-600 text-white font-semibold py-4 rounded-full hover:opacity-90 transition disabled:opacity-40">Interpretar</button>
          </div>
        )}
        {fase === 'resultado' && (
          <div className="flex flex-col gap-4">
