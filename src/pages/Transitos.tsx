...
export default function Transitos() {
  ...
  const generarLectura = async () => {
    ...
    const result = await llamarGemini({
      ...
      prompt: `Eres experto en Tránsitos Astrales. Nombre: ${nombre}, Signo: ${signo}, Fecha de nacimiento: ${fechaNacimiento}. Genera una lectura profunda y personalizada de 3-4 párrafos. Reflexivo, simbólico, poético. Sin predicciones absolutas.`,
    ...
    if (!result.error && result.texto) {
      ...
      supabase.from('ai_cache').insert({
        cache_key: cacheKey, herramienta: 'transitos', prompt_hash: cacheKey,
