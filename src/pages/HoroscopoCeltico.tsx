...
export default function HoroscopoCeltico() {
  ...
  const { registrarApertura, registrarLectura, registrarValoracion } = useAnalytics('horoscopo-celtico', esPremium)
  ...
  const generarLectura = async () => {
    ...
    const result = await llamarGemini({
      ...
      prompt: `Experto en el horóscopo celta de los árboles (reconstrucción moderna). Signo: ${signo}, Nacimiento: ${fechaNacimiento}. Árbol celta y su mensaje. 3 párrafos.`,
    ...
    if (!result.error && result.texto) {
      ...
      supabase.from('horoscopo_cache').insert({
        signo: signo.toLowerCase(), fecha: fechaHoy, tipo: 'horoscopo-celtico',
      ...
      <div className="relative z-10 w-full max-w-sm mx-auto flex flex-col px-6 py-10 gap-6">
        <div className="flex items-center">
          ...
          <div className="flex-1 text-center">
            ...
            <p className="text-purple-300 text-xs">Los 13 árboles</p>
          </div>
        </div>
        <div className="bg-white/5 border border-white/10 rounded-3xl p-5 backdrop-blur text-center">
          <p className="text-purple-300 text-xs tracking-widest uppercase mb-1">Horóscopo Celta</p>
        ...
        {!cargando && interpretacion && (
          <>
            ...
            <Compartir titulo="Horóscopo Celta" texto={interpretacion} hashtags={['Universe', 'HoroscopoCeltico']} />
