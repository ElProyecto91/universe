...
import { calcularNumerologiaNombre } from '../lib/motores/numerologianombre'
...
export default function NumerologiaNombre() {
  ...
  const generarLectura = async () => {
    ...
    const result = await llamarGemini({
      ...
      prompt: `Experto en Numerología del Nombre. Nombre: ${nombre}, Signo: ${signo}, Nacimiento: ${fechaNacimiento}. Genera una lectura profunda de 3-4 párrafos. Reflexivo, simbólico. Sin predicciones absolutas. Máximo 400 palabras.`,
    ...
    if (!result.error && result.texto) {
      ...
      setFromCache(false)
      supabase.from('ai_cache').insert({ cache_key: cacheKey, herramienta: 'numerologia-nombre', prompt_hash: cacheKey, respuesta: result.texto, tokens_used: result.tokensUsados, expires_at: null }).then(() => {})
    } else {
