...
export default function Numerologia() {
  ...
  const cacheKey = `numerologia-${fechaNacimiento}-${añoActual}`
  const userId = null
  ...
  const generarLectura = async () => {
    ...
    } catch (err) { console.warn('[Numerologia] Error caché:', err) }

    const result = await llamarGemini({
      herramienta: 'numerologia',
      prompt: `Experto en Numerología. Nombre: ${nombre}, Signo: ${signo}, Nacimiento: ${fechaNacimiento}. Genera una lectura profunda de 3-4 párrafos. Reflexivo, simbólico. Sin predicciones absolutas. Máximo 400 palabras.`,
    ...
    if (!result.error && result.texto) {
      ...
      setFromCache(false)
      supabase.from('ai_cache').insert({ cache_key: cacheKey, herramienta: 'numerologia', prompt_hash: cacheKey, respuesta: result.texto, tokens_used: result.tokensUsados, expires_at: null }).then(() => {})
    } else {
    ...
    <div className="min-h-screen text-white flex flex-col relative" style={bgStyle}>
      ...
      <div className="relative z-10 w-full max-w-sm mx-auto flex flex-col px-6 py-10 gap-6">
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-6 backdrop-blur text-center">
          <p className="text-purple-300 text-xs tracking-widest uppercase mb-2">Numerología</p>
        ...
        {!cargando && interpretacion && <Compartir titulo="Numerología" texto={interpretacion} hashtags={['Universe', 'Numerologia']} />}
