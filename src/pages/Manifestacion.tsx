...
export default function Manifestacion() {
  ...
  const { registrarApertura, registrarLectura, registrarPaywall, registrarValoracion } = useAnalytics('manifestacion', esPremium)
  ...
  if (!cargandoPlan && !esPremium) {
    registrarPaywall()
    return <Paywall motivo="herramienta" herramienta="Manifestación" />
  }
  ...
  const consultar = async () => {
    ...
    const result = await llamarGemini({
      ...
      prompt: `Guía de manifestación consciente. Nombre: ${nombre}, Signo: ${signo}. Intención: "${pregunta}". 2 párrafos: reencuadre como si ya es realidad + 3 pasos prácticos.`,
