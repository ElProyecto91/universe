...
import { calcularTzolkin } from '../lib/motores/tzolkin'
...
export default function Tzolkin() {
  ...
  const cacheKey = `tzolkin-${fechaNacimiento}-${añoActual}`
  const userId = null
  ...
  const generarLectura = async () => {
    ...
    } catch (err) { console.warn('[Tzolkin] Error caché:', err) }

    const result = await llamarGemini({
      herramienta: 'tzolkin',
      prompt: `Experto en Tzolkín Maya. Nombre: ${nombre}, Signo: ${signo}, Nacimiento: ${fechaNacimiento}. Genera una lectura profunda de 3-4 párrafos. Reflexivo, simbólico. Sin predicciones absolutas. Máximo 400 palabras.`,
    ...
    if (!result.error && result.texto) {
      ...
      setFromCache(false)
      supabase.from('ai_cache').insert({ cache_key: cacheKey, herramienta: 'tzolkin', prompt_hash: cacheKey, respuesta: result.texto, tokens_used: result.tokensUsados, expires_at: null }).then(() => {})
    } else {
    ...
    <div className="min-h-screen text-white flex flex-col relative" style={bgStyle}>
      ...
      <div className="relative z-10 w-full max-w-sm mx-auto flex flex-col px-6 py-10 gap-6">
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-6 backdrop-blur text-center">
          <p className="text-purple-300 text-xs tracking-widest uppercase mb-2">Tzolkín Maya</p>
        ...
        {!cargando && interpretacion && <Compartir titulo="Tzolkín Maya" texto={interpretacion} hashtags={['Universe', 'Tzolkin']} />}
