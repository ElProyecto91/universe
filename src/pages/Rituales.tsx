...
export default function Rituales() {
  ...
  const consultar = async () => {
    ...
    try {
      const prompt = [
        'Eres una guía de rituales estacionales de tradición pagana moderna.',
        'Responde SOLO con texto en español, en prosa continua. Sin asteriscos, sin guiones, sin numeración, sin markdown.',
        '',
        `El usuario se llama ${nombre}, signo ${signo}. Su intención para el ritual es: "${pregunta}".`,
        '',
        'Escribe una guía de ritual de 3 párrafos que fluyan naturalmente.',
        'Párrafo 1: describe el propósito del ritual y cómo preparar el espacio sagrado.',
        'Párrafo 2: describe los pasos del ritual en sí, con los elementos naturales recomendados.',
        'Párrafo 3: describe cómo cerrar el ritual y anclar la intención en el día a día.',
        '',
        'Tono ceremonial y cálido. Separa párrafos con línea en blanco. Termina en punto.',
      ].join('\n')
      ...
      if (!result.error && result.texto) {
        ...
        if (!lecturaGuardadaRef.current) {
          ...
          await guardarLectura({ herramienta: HERRAMIENTA, titulo: `Ritual · ${fechaHoy}`, contenido: `Intención: "${pregunta}"\n\n${result.texto}`, metadatos: { pregunta, fecha: fechaHoy, nombre, signo } })
        }
      } else { setErrorMsg(result.error || 'El universo guarda silencio. Inténtalo de nuevo.') }
    } catch (err) { console.error('[Rituales]', err); setErrorMsg('Error inesperado.') }
      ...
      <div className="flex flex-col gap-6">
        <div className="flex items-center">
          ...
          <div className="flex-1 text-center">
            <p className="text-white font-semibold text-sm">Rituales Estacionales</p>
        ...
        {fase === 'preguntar' && (
          <div className="flex flex-col gap-4">
            <div className="bg-[#0d0015] border border-white/15 rounded-3xl p-6">
              <p className="text-purple-400 text-xs tracking-widest uppercase mb-3">Tu intención</p>
              <textarea value={pregunta} onChange={e => setPregunta(e.target.value)} placeholder="¿Cuál es la intención de tu ritual?" rows={4} className="w-full bg-transparent text-white text-sm resize-none outline-none placeholder-white/30" />
            </div>
            <DisclaimerIA compact />
            <button onClick={consultar} disabled={!pregunta.trim()} className="w-full bg-gradient-to-r from-purple-600 to-pink-600 text-white font-semibold py-4 rounded-full hover:opacity-90 transition disabled:opacity-40">Crear mi ritual</button>
          </div>
        )}
        {fase === 'resultado' && (
          <div className="flex flex-col gap-4">
            ...
            <div className="bg-[#0d0015] border border-white/15 rounded-3xl p-6">
              <p className="text-purple-400 text-xs tracking-widest uppercase mb-4">Tu ritual</p>
              {cargando ? (
            ...
            {!cargando && interpretacion && (
              <>
                ...
                <Compartir titulo="Rituales Estacionales" texto={interpretacion} hashtags={['Universe', 'Rituales']} />
                ...
                <button onClick={() => { setFase('preguntar'); setInterpretacion(''); setErrorMsg(''); lecturaGuardadaRef.current = false }} className="w-full text-purple-300/60 text-sm py-2">Nuevo ritual</button>
              </>
            )}
          </div>
        )}
