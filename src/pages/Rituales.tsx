...
export default function Rituales() {
  ...
  const { registrarApertura, registrarLectura, registrarPaywall, registrarValoracion } = useAnalytics('rituales', esPremium)
  ...
  if (!cargandoPlan && !esPremium) {
    registrarPaywall()
    return <Paywall motivo="herramienta" herramienta="Rituales Estacionales" />
  }
  ...
  const consultar = async () => {
    ...
    const result = await llamarGemini({
      ...
      prompt: `Guía de rituales estacionales (tradición pagana moderna). Nombre: ${nombre}, Signo: ${signo}. Intención del ritual: "${pregunta}". Ritual simple de 3 pasos.`,
