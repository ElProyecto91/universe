...
export default function Compartir({ titulo, texto, hashtags = [] }: Props) {
  ...
  const generarImagenYCompartir = async () => {
    try {
      ...
      const LINE_H_TIT  = 60
      const LINE_H_TXT  = 52
      const MAX_W       = W - PAD * 2 - CARD_PAD_X * 2
      ...
      t.font = FONT_TITULO
      const nLinTitulo = contarLineas(t, titulo, MAX_W)

      t.font = FONT_TEXTO
      const nLinTexto = contarLineasParrafos(t, texto, MAX_W)

      // Calcular altura dinámica
      const yLogo     = PAD + 80
      const ySep1     = yLogo + 30
      const yTitulo   = ySep1 + 60
      const ySep2     = yTitulo + nLinTitulo * LINE_H_TIT + 30
      const yTexto    = ySep2 + 50
      const yTextoFin = yTexto + nLinTexto * LINE_H_TXT
      ...
      ctx.textAlign = 'center'
      dibujarTextoWrap(ctx, titulo, W / 2, yTitulo, MAX_W, LINE_H_TIT, 'center')
      ...
      ctx.font = FONT_TEXTO
      ctx.textAlign = 'left'
      dibujarTextoParrafos(ctx, texto, cardX + CARD_PAD_X, yTexto, MAX_W, LINE_H_TXT)
      ...
      // Generar blob y compartir con selector nativo
      ...
      // Fallback texto plano con enlace
      if (navigator.share) {
        await navigator.share({
          title: titulo,
  ...
  return (
    <div className="w-full bg-[#0d0015] border border-white/15 rounded-3xl p-5">
      ...
      <button
        ...
        className="w-full bg-white/5 border border-white/10 text-white/50 rounded-2xl py-3 text-xs hover:bg-white/10 transition"
      >
        Copiar enlace
      </button>
    </div>
...
function dibujarTextoParrafos(
  ...
  for (const p of parrafos) {
    ...
    for (const word of words) {
      const test = line + word + ' '
      if (ctx.measureText(test).width > maxW && line !== '') {
        ctx.fillText(line.trim(), x, y + total * lineH)
        line = word + ' '
        total++
      } else {
        line = test
